        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
          <div>
            ...
            <p className="text-sm text-text-muted mt-1">
              Pedidos completados (excluyendo cancelados)
            </p>
          </div>
...
function calcularStats(pedidos, lineas, diasRango) {
  // Resumen
  ...
  // Top productos (por cantidad de unidades pedidas)
  ...
  for (const l of lineas) {
    ...
    prev.ingresos += Number(l.cantidad || 0) * Number(l.precio || 0);
    prev.pedidos += 1;
  ...
  const peoresProductos = productos.slice(-5).reverse();
  ...
  for (const p of pedidos) {
    ...
    if (porDia.has(clave)) porDia.set(clave, porDia.get(clave) + 1);
  }
  ...
  // Clientes recurrentes (que aparecen más de 1 vez)
...
function SeccionTopProductos({ stats }) {
  ...
  const maxUds = stats.topProductos[0].unidades;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-10">
      ...
      <div className="card p-5 sm:p-6">
        ...
        <p className="text-xs text-text-muted mb-5">Productos estrella</p>
        <div className="space-y-2">
          {stats.topProductos.map((p, i) => (
            <div key={p.nombre} className="flex items-center gap-3">
              <span className="text-xs font-bold w-5 text-right text-text-muted tabular-nums">
                {i + 1}
              </span>
      ...
      {stats.peoresProductos.length > 0 && (
        <div className="card p-5 sm:p-6">
          ...
          <p className="text-xs text-text-muted mb-5">Candidatos a quitar de la carta</p>
          <div className="space-y-2">
    ...
    <div className="card p-5 sm:p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <InsightItem
          ...
          label="Hora pico"
          valor={horaPicoStr}
          descripcion="Cuando recibes más pedidos"
        />
        <InsightItem
          icon={Calendar}
          label="Día más fuerte"
          valor={diaPicoStr}
          descripcion="El que más volumen mueve"
        />
        {stats.tiempoMedioPrep !== null && (
          <InsightItem
            icon={Clock}
            label="Tiempo medio de preparación"
            valor={stats.tiempoMedioPrep + ' min'}
            descripcion="De recibido a entregado"
          />
        )}
        <InsightItem
