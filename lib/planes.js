...
// ============================================================
//
// IMPORTANTE: la mensualidad y el precio por pedido de más llevan el IVA
// INCLUIDO (de 99 € se ingresan 81,82 €). La implementación NO: 120 € + IVA.
// Dos criterios distintos a propósito; precios oficiales cerrados por Sandra
// el 2026-09-11. Donde se muestren estos importes, poner "IVA incluido".
...
// ============================================================
// `incluidosOverride` = restaurantes.pedidos_incluidos. Si viene con valor,
// manda sobre el del plan (limites pactados a medida con un cliente).
export function calcularConsumo({ planId, pedidosPeriodo, diasTranscurridos, diasTotales, incluidosOverride }) {
  const plan = infoPlan(planId);
  const pactados = Number(incluidosOverride);
  const incluidos = (isFinite(pactados) && pactados > 0) ? Math.round(pactados) : plan.pedidosIncluidos;
